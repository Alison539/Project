...
def generateRepetitionCode(distance: int, flip_error_prob: float, depolarization_error_prob: float):
    ...
    for i in range((-distance) + 1, 0):
        code.append("DETECTOR", [stim.target_rec(i)], [((distance - 1 + i)*2) + 1, 0])
    ...
    for i in range((-distance) + 1, 0):
        code.append("DETECTOR", [stim.target_rec(i), stim.target_rec(i-1), stim.target_rec(i-distance)], [(distance - 1 + i)*2 + 1, 1])
